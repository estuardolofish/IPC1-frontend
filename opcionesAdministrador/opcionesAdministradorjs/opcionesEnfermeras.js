...
{
    var tabla = document.querySelector('#tEnfermera');
    var cadena = ``;
    fetch('https://proyecto2-backend-ipc1.herokuapp.com/Usuarios', {
    method: 'GET',
    headers:{
        ...
        for(var i in response)
        {
            ...
            `
                <tr>
                    ...
                    <td> 
                    <button type="button" onclick="accionesEnfermera(this)" class="fa fa-pencil-square-o btn btn-success btnAccionesEnfermera" aria-hidden="true" data-toggle="modal" data-target="#modalEditarEnfermera"  value=${response[i].UserName}></button> 
                    </td>
                </tr>
            `
...
function accionesEnfermera(boton) {
...
function modificarEnfermera() {
    ...
    console.log(localStorage.getItem("userAuxiliar"));
     ...
     .then(response =>{
         ...
         console.log(response);
         location.href = "../opcionesAdministrador/cargarEnfermeras.html";
...
function PDFreporte() {
    ...
    pdf.text(25,25,"Reporte Enfermeras");
